...
<title>ProjX v3 | Simulation Lab</title>
<link href="https://fonts.googleapis.com/css2?family=Share+Tech+Mono&family=Orbitron:wght@700;900&family=JetBrains+Mono:wght@400;500;600&display=swap" rel="stylesheet">
<style>
:root{
  --bg:#080a0f;--surface:#0d1017;--panel:#111520;--border:#1e2535;
  --glow:#00ffe7;--glow2:#ff4d6d;--yellow:#ffe156;--dim:#4a5568;
  --text:#c9d6e8;--green:#39ff8f;--orange:#ff9f43;--purple:#a29bfe;
}
...
body{
  ...
  background-image:radial-gradient(ellipse 80% 50% at 50% -10%,rgba(0,255,231,.05) 0%,transparent 60%);
}
header{
...
.logo{font-family:'Orbitron',sans-serif;font-size:1.2rem;font-weight:900;letter-spacing:2px;color:#fff;flex-shrink:0}
.logo em{color:var(--glow);font-style:normal;text-shadow:0 0 12px var(--glow)}
...
.tab-btn{flex-shrink:0;background:transparent;border:1px solid var(--border);color:var(--dim);
  padding:5px 11px;cursor:pointer;border-radius:3px;font-size:.68rem;
  font-family:'Share Tech Mono',monospace;transition:all .18s;white-space:nowrap}
...
.tab-btn.fork-tab.active{border-color:var(--purple);color:var(--purple);background:rgba(162,155,254,.08);box-shadow:0 0 8px rgba(162,155,254,.2)}
...
.run-btn{background:var(--glow);color:#000;border:none;padding:7px 16px;border-radius:3px;
  font-family:'Orbitron',sans-serif;font-size:.62rem;font-weight:700;letter-spacing:1px;
  cursor:pointer;transition:all .18s;box-shadow:0 0 14px rgba(0,255,231,.35);flex-shrink:0}
.run-btn:hover{background:#fff;box-shadow:0 0 22px rgba(0,255,231,.6);transform:scale(1.04)}
...
.canvas-label{position:absolute;top:10px;left:14px;font-size:.62rem;color:var(--dim);letter-spacing:1px;pointer-events:none;z-index:2}
...
.panel-title{font-size:.58rem;color:var(--glow);letter-spacing:2px;text-transform:uppercase;
...
.q-card{background:var(--bg);border-left:2px solid var(--glow);border-radius:0 3px 3px 0;
...
.q-label{font-size:.58rem;color:var(--dim);text-transform:uppercase;letter-spacing:.5px;margin-bottom:2px;word-break:break-all}
.q-value{font-size:.9rem;color:var(--green);font-weight:bold}
.q-value.fail{color:var(--glow2)}
.q-unit{font-size:.65rem;color:var(--dim);margin-left:3px}
.q-note{font-size:.58rem;color:var(--orange);margin-top:2px;word-break:break-word}
...
.env-key{color:var(--dim)}.env-val{color:var(--yellow)}
#legend{display:flex;flex-wrap:wrap;gap:7px;padding:4px 0 0}
.legend-item{display:flex;align-items:center;gap:5px;font-size:.62rem;color:var(--text)}
.legend-dot{width:14px;height:3px;border-radius:2px}
#tooltip{position:fixed;background:rgba(8,10,15,.97);border:1px solid var(--glow);
  border-radius:4px;padding:7px 10px;font-size:.68rem;pointer-events:none;display:none;
...
#tooltip .tt-id{font-size:.62rem;margin-bottom:3px}
...
.anim-progress{position:absolute;bottom:0;left:0;right:0;height:2px;
  background:rgba(0,255,231,.1);border-radius:0 0 6px 6px;overflow:hidden;z-index:3}
.anim-progress-bar{height:100%;background:var(--glow);box-shadow:0 0 8px var(--glow);
  transition:width .05s linear;width:0%}
.crosshair-h,.crosshair-v{position:absolute;pointer-events:none;opacity:0;transition:opacity .1s}
...
<header>
  <div class="logo">Proj<em>X</em><sup> v3</sup></div>
  <div id="tabs"></div>
...
<main>
  ...
  <aside class="sidebar">
    ...
    <div class="panel"><div class="panel-title">Legend</div><div id="legend"></div></div>
  </aside>
</main>
<script>
/* ── injected data ── */
    ...
    if(s.label&&s.label.match(/^Fork:/)){
      ...
      const projs=grp.map(g2=>{
        ...
        const p=g2.projectiles&&g2.projectiles[0]?{...g2.projectiles[0]}:
          {id:'p',angle:45,speed:50,launch_from:[0,0],mass:null,drag_coeff:null,cross_section:null};
      ...
      out.push({mode:'fork',
        ...
        gravity:grp[0].gravity,air_resistance:false,air_density:1.225,wind_x:0,wind_y:0,
...
function drawGrid(){
  ...
  ctx.strokeStyle='rgba(30,37,53,.85)';ctx.lineWidth=1;
  ...
  ctx.fillStyle='rgba(74,85,104,.85)';
  ...
  ctx.strokeStyle='rgba(0,255,231,.22)';ctx.lineWidth=1.5;
  ctx.beginPath();ctx.moveTo(ox,PT);ctx.lineTo(ox,oy);ctx.lineTo(W-PR,oy);ctx.stroke();
  ctx.fillStyle='rgba(0,255,231,.45)';ctx.font='10px Share Tech Mono,monospace';
...
function renderSim(){
  ...
  [...trajectories,...bounceTrajs].forEach(tr=>{
    const lim=isAnim?Math.min(af,tr.points.length):tr.points.length;
    drawTraj(tr,lim);
    /* moving object at current frontier */
    const oi=isAnim?Math.min(af,tr.points.length)-1:tr.points.length-1;
    if(oi>=0){
  ...
  if(session.mode==='fork'&&!isAnim){
    trajectories.forEach(tr=>{
      ...
      ctx.save();ctx.fillStyle=tr.color;ctx.font='bold 10px Share Tech Mono';
...
function loadSession(s){
  ...
  (s.projectiles||[]).forEach(p=>{
    ...
    trajectories.push({id:p.id,color:col,points:pts,branchLabel:p.branchLabel||null});
  });
  (s.bounces||[]).forEach(b=>{
    const bg=b.branchGravity!=null?b.branchGravity:g;
    (b.arcs||[]).forEach(arc=>{
      const pts=buildBounceTraj(arc,bg);
      bounceTrajs.push({id:b.p+'_bounce',color:colorFor(b.p),points:pts,dashed:true});
    });
  });
  (s.collisions||[]).forEach(c=>collisionPts.push({x:c.x,y:c.y,label:c.p1+'x'+c.p2}));
  totF=Math.max(...[...trajectories,...bounceTrajs].map(t=>t.points.length),1);
...
function handleSimHover(e){
  ...
  [...trajectories,...bounceTrajs].forEach(tr=>
    tr.points.forEach(p=>{
      ...
      if(d<minD){minD=d;cl={...p,id:tr.id,color:tr.color};}
    })
  );
  if(cl){
    tooltip.style.display='block';
    tooltip.style.left=(e.clientX+16)+'px';tooltip.style.top=(e.clientY-50)+'px';
    document.getElementById('tt-id').textContent=cl.id;
    document.getElementById('tt-id').style.color=cl.color||'var(--glow)';
    document.getElementById('tt-x').textContent=cl.x.toFixed(3);
    document.getElementById('tt-y').textContent=cl.y.toFixed(3);
    document.getElementById('tt-t').textContent=cl.t.toFixed(3);
...
function updateSidebar(s){
  ...
  if(s.mode==='game'){
    ...
    leg.innerHTML=`<div class="legend-item">
      ...
      <div style="font-size:.6rem;color:var(--dim);margin-top:8px;line-height:1.8">
    ...
    if(shapes.length)leg.innerHTML+=
      ...
      shapes.map(sh=>`<div style="font-size:.57rem;color:var(--dim);padding:2px 0">${sd[sh]||sh}</div>`).join('')+'</div>';
  }
  ...
  const qs=s.queries||[];
  if(!qs.length){mc.innerHTML='<div style="color:var(--dim);font-size:.7rem">No queries in this block.</div>';return;}
