...
let jobj pairs =
  let members = List.filter_map (fun (k, v_opt) ->
    match v_opt with
    | None   -> None
    | Some v -> Some (js k ^ ":" ^ v)
  ) pairs in
...
and je_dotq env = function
  ...
  | DotCollide (p1, p2, g_opt) ->
      ...
      let (hit,_,_,_) = collide
        ...
        ~angle2:proj2.angle ~speed2:proj2.speed ~x02 ~y02 ~gravity:g in
...
and je_gopt env = function
  | Some e -> je_expr env e
  | None   -> 9.8
...
let proj_to_json name (p : projectile_val) =
  let (x0, y0, _) = p.launch_from in
  jobj [
    "id",            Some (js name);
    "angle",         Some (jf p.angle);
    "speed",         Some (jf p.speed);
    "launch_from",   Some (Printf.sprintf "[%s,%s]" (jf x0) (jf y0));
    "mass",          Some (match p.mass          with Some m -> jf m | None -> "null");
    "drag_coeff",    Some (match p.drag_coeff    with Some d -> jf d | None -> "null");
    "cross_section", Some (match p.cross_section with Some c -> jf c | None -> "null");
  ]

(* Evaluate an inline SProjectile record and register it in env *)
let eval_sprojectile env (p : Ast.sim_stmt) =
  match p with
  | SProjectile sp ->
      let a = je_expr env sp.angle in
      let s = je_expr env sp.speed in
      let (x, y, t) = match sp.launch_from with
        | Some (ex,ey,et) -> (je_expr env ex, je_expr env ey, je_expr env et)
        | None            -> (0.0, 0.0, 0.0)
      in
...
let simulate_to_json env stmts sim_number =
  ...
  (* Pass 1: collect physics settings at top level *)
  ...
  (* proj_jsons: collected at plot-time in Pass 3, one entry per SPlot executed
     (including every loop iteration), preserving the exact projectile state *)
  ...
  let add_query label value unit note =
    queries := !queries @ [jobj [
      "label", Some (js label); "value", Some (jf value);
      "unit",  Some (js unit);
  ...
  let add_query_str label value_str unit note =
    queries := !queries @ [jobj [
      "label", Some (js label); "value", Some (js value_str);
      "unit",  Some (js unit);
      "note",  (match note with Some n -> Some (js n) | None -> None);
    ]]
  in

  (* Pass 3: execute — threads env so inline projectiles are visible *)
  let rec exec env_inner stmt =
    ...
    (* inline projectile — register and continue *)
    | SProjectile sp -> eval_sprojectile env_inner (SProjectile sp)

    | SPlot pid ->
        let pv          = get_projectile pid env_inner.projectiles in
        let (x0, y0, _) = pv.launch_from in
        let (mass,cd,area) = get_drag_params pv in
        let r  = if ar then range_with_drag pv.angle pv.speed g x0 y0 dp mass cd area
                 else range pv.angle pv.speed g +. x0 in
        let mh = if ar then max_height_with_drag pv.angle pv.speed g y0 dp mass cd area
                 else max_height pv.angle pv.speed g +. y0 in
        (* snapshot this projectile's current state for the projectiles array *)
        proj_jsons := !proj_jsons @ [proj_to_json pid pv];
        annotations := !annotations @ [
          jobj ["type", Some (js "range");      "p", Some (js pid); "value", Some (jf r)];
          jobj ["type", Some (js "max_height"); "p", Some (js pid); "value", Some (jf mh)];
        ];
        env_inner
    ...
    | SMaxRange pid ->
        ...
        let (mass,cd,area) = get_drag_params pv in
        if ar then begin
          let (mr, opt_ang) = max_range_with_drag pv.speed g x0 y0 dp mass cd area in
    ...
    | SMaxHeight pid ->
        let pv         = get_projectile pid env_inner.projectiles in
        let (_, y0, _) = pv.launch_from in
        let (mass,cd,area) = get_drag_params pv in
        let mh = if ar then max_height_with_drag pv.angle pv.speed g y0 dp mass cd area
                 else max_height pv.angle pv.speed g +. y0 in
        add_query ("max_height " ^ pid) mh "m" (if ar then Some "with drag" else None);
        env_inner
    ...
    | SCollide (p1, p2) ->
        ...
        let (x01,y01,_) = pv1.launch_from in
        let (x02,y02,_) = pv2.launch_from in
        let (hit,t,cx,cy) = collide
          ~angle1:pv1.angle ~speed1:pv1.speed ~x01 ~y01
          ~angle2:pv2.angle ~speed2:pv2.speed ~x02 ~y02 ~gravity:g in
        if hit then begin
          collisions := !collisions @ [jobj [
            "p1", Some (js p1); "p2", Some (js p2);
            "t",  Some (jf t);  "x",  Some (jf cx); "y", Some (jf cy);
          ]];
          add_query_str ("collide " ^ p1 ^ "/" ^ p2)
            (Printf.sprintf "YES  t=%.2fs  (%.1f, %.1f)" t cx cy) "" None
        end else
          add_query_str ("collide " ^ p1 ^ "/" ^ p2) "NO" "" None;
        env_inner
    ...
    | SCollisionVel (p1, p2) ->
        ...
        if hit then begin
          let ((vx1,vy1),(vx2,vy2)) = collision_vel
            ~angle1:pv1.angle ~speed1:pv1.speed
            ~angle2:pv2.angle ~speed2:pv2.speed ~gravity:g t in
          add_query_str ("collision_vel " ^ p1)
            (Printf.sprintf "(%.2f, %.2f)" vx1 vy1) "m/s" None;
          add_query_str ("collision_vel " ^ p2)
            (Printf.sprintf "(%.2f, %.2f)" vx2 vy2) "m/s" None
        end else
          add_query_str ("collision_vel " ^ p1 ^ "/" ^ p2) "no collision" "" None;
        env_inner
    ...
    | SBounce (pid, n_e, r_e) ->
        ...
        let arc_jsons   = List.map (fun (ax,ay,ang,spd) ->
          Printf.sprintf "[%s,%s,%s,%s]" (jf ax) (jf ay) (jf ang) (jf spd)) arcs in
        bounces := !bounces @ [jobj ["p", Some (js pid); "arcs", Some (jarr arc_jsons)]];
        let hop_lines = List.mapi (fun i (x,_,ang,spd) ->
          Printf.sprintf "hop %d: x0=%.2f  range=%.2f m" (i+1) x (range ang spd g)
        ) arcs in
    ...
    | SCheck c ->
        add_query_str "check"
          (if je_cond env_inner c then "PASS ✓" else "FAIL ✗") "" None;
        env_inner
    ...
    | SFor (var, s_e, e_e, step_e, body) ->
        ...
        let rec loop i cur =
          if i > ev_ then cur
            ...
            loop (i +. step)
              { after with vars = List.filter (fun (n,_) -> n <> var) after.vars }
        in
        loop sv env_inner
    ...
    | SRepeat (n_e, body) ->
        ...
        let rec loop i cur =
          if i <= 0 then cur
          else loop (i-1) (List.fold_left exec cur body)
        in
        loop n env_inner
  ...
  let label =
    ...
    if ar then base ^ Printf.sprintf "  air  ρ=%.3f  wind=(%.1f,%.1f)"
                        dp.air_density dp.wind_x dp.wind_y
    else base
  in
...
let fork_branch_to_json env proj_name (pv : projectile_val) branch =
  ...
  let rec collect_settings env_inner s =
    match s with
    | SGravity e            -> gravity := je_expr env_inner e; env_inner
    | SBounce (_, n_e, r_e) -> bounce_args := Some (n_e, r_e); env_inner
    | SProjectile sp        -> eval_sprojectile env_inner (SProjectile sp)
    | SFor (var, s_e, e_e, step_e, body) ->
        let sv   = je_expr env_inner s_e in
        let ev_  = je_expr env_inner e_e in
  ...
  let annotations = [
    jobj ["type", Some (js "range");      "p", Some (js proj_name); "value", Some (jf r)];
    jobj ["type", Some (js "max_height"); "p", Some (js proj_name); "value", Some (jf mh)];
  ] in

  let bounces = match !bounce_args with
    | None -> []
    | Some (n_e, r_e) ->
...
let rec collect env stmts sim_counter acc =
  ...
  | Projectile { name; angle; speed; launch_from; mass; drag_coeff; cross_section } :: rest ->
      ...
      in
      let pv : projectile_val = {
        angle = a; speed = s; launch_from = (x, y, t);
  ...
  | Fork (proj_name, branches) :: rest ->
      ...
      let scens_rev = List.rev scens in
      collect env rest sim_counter (scens_rev @ acc)

  | Game { planet; level; lives } :: rest ->
      let scen = game_to_json env planet level lives in
      collect env rest sim_counter (scen :: acc)
      ...
      let (inner_acc, _env2, n2) =
        let rec loop i cur_env ctr accum =
          if i > ev_ then (accum, cur_env, ctr)
            ...
            loop (i +. step) env_after (ctr + List.length new_scens) (accum @ new_scens)
        in
      ...
      let (inner_acc, env2, n2) =
        let rec loop i cur_env ctr accum =
          if i <= 0 then (accum, cur_env, ctr)
            ...
            loop (i-1) env_after (ctr + List.length new_scens) (accum @ new_scens)
        in
...
let emit_json (program : Ast.program) : string =
  let (ready_env, _) =
    Eval.eval_stmts_and_collect Env.empty_env program
  in
