(* ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
   Simulate block
   ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━ *)
(* ── ProjX v3 Evaluator with Air Resistance ── *)
...
and eval_simulate env stmts =
  ...
  Printf.printf "\n── simulate (g=%.2f" !gravity;
  if !air_resistance then
    Printf.printf ", air_resistance=ON, ρ=%.3f, wind=(%.1f,%.1f)" 
      !air_density !wind_x !wind_y;
  Printf.printf ") ──\n";
  ...
  List.iter (fun s ->
    ...
    | SPlot p ->
        ...
        if !air_resistance then begin
          let (mass, cd, area) = get_drag_params proj in
          
          if Option.is_none proj.mass && Option.is_none proj.drag_coeff && 
             Option.is_none proj.cross_section then
            Printf.printf "  (using default drag params: mass=%.2fkg, Cd=%.2f, A=%.4fm²)\n"
              mass cd area;
          
          let r = range_with_drag proj.angle proj.speed !gravity x0 y0 
                    drag_params mass cd area in
          let mh = max_height_with_drag proj.angle proj.speed !gravity y0 
                     drag_params mass cd area in
          
          Printf.printf "plot %s (WITH DRAG): range=%.2f m  max_height=%.2f m\n" p r mh;
          
          let pts = arc_points_with_drag proj.angle proj.speed !gravity x0 y0 50 
                      drag_params mass cd area in
          ignore pts
        end else begin
    ...
    | SRange p ->
        ...
        let r = if !air_resistance then begin
          let (mass, cd, area) = get_drag_params proj in
          range_with_drag proj.angle proj.speed !gravity x0 y0 drag_params mass cd area
        end else
          range proj.angle proj.speed !gravity +. x0
        in
        Printf.printf "range %s = %.4f m%s\n" p r 
          (if !air_resistance then " (with drag)" else "")
    ...
    | SMaxRange p ->
        ...
        if !air_resistance then begin
          let (mass, cd, area) = get_drag_params proj in
          let (mr, opt_ang) = max_range_with_drag proj.speed !gravity x0 y0 
                                drag_params mass cd area in
          Printf.printf "max_range %s = %.4f m at %.1f° (with drag)\n" p mr opt_ang
        end else begin
    ...
    | SMaxHeight p ->
        ...
        let mh = if !air_resistance then begin
          let (mass, cd, area) = get_drag_params proj in
          max_height_with_drag proj.angle proj.speed !gravity y0 drag_params mass cd area
        end else
          max_height proj.angle proj.speed !gravity +. y0
        in
        Printf.printf "max_height %s = %.4f m%s\n" p mh 
          (if !air_resistance then " (with drag)" else "")
...
and eval_stmt env stmt =
  ...
  | Projectile { name; angle; speed; launch_from; mass; drag_coeff; cross_section } ->
      ...
      let m_opt = Option.map (eval_expr env) mass in
      let d_opt = Option.map (eval_expr env) drag_coeff in
      let c_opt = Option.map (eval_expr env) cross_section in
      ...
      Printf.printf "projectile %s: angle=%.1f° speed=%.1f m/s" name a s;
      if Option.is_some m_opt then
        Printf.printf " mass=%.3f kg" (Option.get m_opt);
      if Option.is_some d_opt then
        Printf.printf " Cd=%.2f" (Option.get d_opt);
      if Option.is_some c_opt then
        Printf.printf " A=%.4f m²" (Option.get c_opt);
      Printf.printf "\n";
