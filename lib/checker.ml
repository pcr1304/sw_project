...
and check_sim_stmts env stmts =
  ...
  let rec count s =
    ...
    | SFor (_,_,_,_,body)
    | SRepeat (_,body)
    | SWhile (_,body)                       -> List.iter count body
    | _                                     -> ()
  in
  ...
  let rec check_sim_stmt_r env s =
    ...
    | SProjectile p ->
        ...
        Option.iter (check_expr env) p.azimuth;   (* NEW: check azimuth expr if present *)
        check_expr env p.speed;
        Option.iter (fun (x,y,z,t) ->
          check_expr env x; check_expr env y; check_expr env z; check_expr env t) p.launch_from;
...
and check_stmt env = function
  | Projectile p ->
      check_expr env p.angle;
      Option.iter (check_expr env) p.azimuth;    (* NEW *)
      check_expr env p.speed;
      Option.iter (fun (x,y,z,t) ->
        check_expr env x; check_expr env y; check_expr env z; check_expr env t) p.launch_from;
      Option.iter (check_expr env) p.mass;
  ...
  | For (var, a, b, step, body) ->
      check_expr env a; check_expr env b; check_expr env step;
  ...
  | IfElse (c, tbody, fbody_opt) ->
      ...
      Option.iter (fun fb -> ignore (List.fold_left check_stmt env fb)) fbody_opt;
      env
