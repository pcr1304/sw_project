...
let json_of_points3d pts =
  `List (List.map (fun (_t, x, y, z) -> `List [`Float x; `Float y; `Float z]) pts)
...
let rec build_proj_val env_inner angle azimuth speed launch_from mass drag_coeff cross_section =
  let a = eval_expr env_inner angle in
  let az = match azimuth with
    | Some e -> eval_expr env_inner e
    | None   -> 0.0
  in
  let s_val = eval_expr env_inner speed in
  let (x, y, z, t) = match launch_from with
    | Some (ex, ey, ez, et) ->
        (eval_expr env_inner ex, eval_expr env_inner ey,
         eval_expr env_inner ez, eval_expr env_inner et)
...
and eval_simulate env stmts =
  ...
  let rec exec_sim env_inner s =
    ...
    | SWindX _ | SWindY _ | SWindZ _ -> env_inner  (* already collected *)

    | SProjectile sp ->
        let proj = build_proj_val env_inner
                     sp.angle sp.azimuth sp.speed sp.launch_from
                     sp.mass sp.drag_coeff sp.cross_section in
    ...
    | SPlot p ->
        ...
        let r  = if !air_resistance then
            Physics.range_with_drag proj.angle proj.speed !gravity x0 y0 drag_params m cd area
          else Physics.range proj.angle proj.speed !gravity +. x0 in
        let mh = if !air_resistance then
            Physics.max_height_with_drag proj.angle proj.speed !gravity y0 drag_params m cd area
          else Physics.max_height proj.angle proj.speed !gravity +. y0 in
        let pts3d = simulate_trajectory_3d
          ~angle:proj.angle ~azimuth:proj.azimuth ~speed:proj.speed
          ~x0 ~y0 ~z0 ~gravity:!gravity
          ~mass:m ~drag_coeff:cd ~cross_section:area
          ~drag_params () in
    ...
    | SRange p ->
        ...
        in
        session_data := `Assoc [("range", `Assoc [("name", `String p); ("value", `Float r)])]
                        :: !session_data;
        env_inner
    ...
    | SMaxRange p ->
        ...
        session_data := `Assoc [("max_range", `Assoc [("name", `String p); ("value", `Float r)])]
                        :: !session_data;
        env_inner
    ...
    | SMaxHeight p ->
        ...
        session_data := `Assoc [("max_height", `Assoc [("name", `String p); ("value", `Float mh)])]
                        :: !session_data;
        env_inner
    ...
    | SMaxRect p ->
        ...
        session_data := `Assoc [("max_rect", `Assoc [("name", `String p); ("value", `Float area)])]
                        :: !session_data;
        env_inner
    ...
    | SMinVel (p, x_e, h_e) ->
        ...
        session_data := `Assoc [("min_vel", `Assoc [("name", `String p); ("value", `Float mv)])]
                        :: !session_data;
        env_inner
    ...
    | SCollisionVel (p1, p2) ->
        ...
        let (x01, y01, _, _) = proj1.launch_from in
        let (x02, y02, _, _) = proj2.launch_from in
        let (_, vx_v, vy_v, _) = collide ~angle1:proj1.angle ~speed1:proj1.speed ~x01 ~y01
                                    ~angle2:proj2.angle ~speed2:proj2.speed ~x02 ~y02 ~gravity:!gravity in
        session_data := `Assoc [("collision_vel", `Assoc [
          ("p1", `String p1); ("p2", `String p2);
          ("vx", `Float vx_v); ("vy", `Float vy_v)
        ])] :: !session_data;
        env_inner
    ...
    | SBounce (p, n_e, r_e) ->
        ...
        let json_arcs = List.map (fun (x, y, ang, spd) ->
          let pts3d = Physics.arc_points ang spd !gravity x y 50
            |> List.map (fun (px,py) -> (0.0,px,py,0.0)) in
          json_of_points3d pts3d) arcs in
        session_data := `Assoc [("bounce", `Assoc [("name", `String p); ("arcs", `List json_arcs)])]
                        :: !session_data;
        env_inner

    | SCheck c ->
        let res = eval_cond env_inner c in
        session_data := `Assoc [("check", `Assoc [("status", `String (if res then "PASS" else "FAIL"))])]
                        :: !session_data;
        env_inner
...
and eval_fork env name branches =
  ...
  let branch_json = List.map (fun br ->
    ...
    let rec exec_br env_inner s =
      match s with
      | SGravity _ | SAirResistance _ | SAirDensity _
      | SWindX _ | SWindY _ | SWindZ _ -> env_inner

      | SProjectile sp ->
          let pv = build_proj_val env_inner
                     sp.angle sp.azimuth sp.speed sp.launch_from
                     sp.mass sp.drag_coeff sp.cross_section in
      ...
      | SPlot p ->
          ...
          let pts3d = simulate_trajectory_3d
            ~angle:proj.angle ~azimuth:proj.azimuth ~speed:proj.speed
            ~x0 ~y0 ~z0 ~gravity:!gravity
            ~mass:m ~drag_coeff:cd ~cross_section:area () in
          let r  = Physics.range proj.angle proj.speed !gravity +. x0 in
          let mh = Physics.max_height proj.angle proj.speed !gravity +. y0 in
          ignore p;
          branch_data := `Assoc [("plot", `Assoc [
            ("name", `String p); ("range", `Float r);
            ("max_height", `Float mh); ("points", json_of_points3d pts3d)
          ])] :: !branch_data;
          env_inner
      ...
      | SRange _ ->
          ...
          branch_data := `Assoc [("range", `Assoc [("name", `String name); ("value", `Float r)])]
                         :: !branch_data;
          env_inner
      ...
      | SBounce (p, n_e, r_e) ->
          ...
          let json_arcs = List.map (fun (x, y, ang, spd) ->
            let pts3d = Physics.arc_points ang spd !gravity x y 50
              |> List.map (fun (px,py) -> (0.0,px,py,0.0)) in
            json_of_points3d pts3d) arcs in
          branch_data := `Assoc [("bounce", `Assoc [("name", `String p); ("arcs", `List json_arcs)])]
                         :: !branch_data;
          env_inner

      | SCheck c ->
          let res = eval_cond env_inner c in
          branch_data := `Assoc [("check", `Assoc [("status", `String (if res then "PASS" else "FAIL"))])]
                         :: !branch_data;
          env_inner
      ...
      | SFor (var, s_e, e_e, step_e, body) ->
          ...
          let rec loop i cur_env =
            if i > end_v then cur_env
            else loop (i +. step_v) (List.fold_left exec_br (add_var var i cur_env) body)
          in
          loop start_v env_inner
      ...
      | SWhile (cond_e, body) ->
          let rec loop cur_env =
            if eval_cond cur_env cond_e then loop (List.fold_left exec_br cur_env body)
            else cur_env
          in
          loop env_inner
    ...
    let pts3d = simulate_trajectory_3d
      ~angle:proj.angle ~azimuth:proj.azimuth ~speed:proj.speed
      ~x0 ~y0 ~z0 ~gravity:!gravity
      ~mass ~drag_coeff:cd ~cross_section:area () in
    `Assoc [
...
and eval_stmt env stmt =
  ...
  | Projectile p ->
      let proj = build_proj_val env p.angle p.azimuth p.speed p.launch_from
                   p.mass p.drag_coeff p.cross_section in
  ...
  | Game g ->
      let json = `Assoc [("type",   `String "game");
                         ("planet", `String g.planet);
                         ("level",  `Float (eval_expr env g.level))] in
      (env, Some json)
