...
let eval_sprojectile env (p : Ast.sim_stmt) =
  ...
  | SProjectile sp ->
      ...
      let (x, y, z, t) = match sp.launch_from with
        | Some (ex,ey,ez,et) -> (je_expr env ex, je_expr env ey,
                                 je_expr env ez, je_expr env et)
...
let simulate_to_json env stmts sim_number =
  ...
  let rec exec env_inner stmt =
    ...
    | SWindX _   | SWindY _         | SWindZ _ -> env_inner   (* NEW: SWindZ here *)
    ...
    | SPlot pid ->
        ...
        let traj3d = simulate_trajectory_3d
          ~angle:pv.angle ~azimuth:pv.azimuth ~speed:pv.speed
          ~x0 ~y0 ~z0 ~gravity:g
          ~mass ~drag_coeff:cd ~cross_section:area
          ~drag_params:dp () in
        let pts3d_json = jarr (List.map (fun (_t,x,y,z) ->
          Printf.sprintf "[%s,%s,%s]" (jf x) (jf y) (jf z)) traj3d) in
        ...
        annotations := !annotations @ [
          ...
          jobj ["type", Some (js "points3d");   "p", Some (js pid); "value", Some pts3d_json];
        ];
        env_inner
    ...
    | SBounce (pid, n_e, r_e) ->
        ...
        let hop_lines = List.mapi (fun i (x,_,ang,spd) ->
          Printf.sprintf "hop %d: x0=%.2f  range=%.2f m" (i+1) x (range ang spd g)) arcs in
  ...
  let label =
    ...
    if ar then base ^ Printf.sprintf "  air  ρ=%.3f  wind=(%.1f,%.1f,%.1f)"
                        dp.air_density dp.wind_x dp.wind_y dp.wind_z
    else base
  in
...
let rec collect env stmts sim_counter acc =
  ...
  | Projectile { name; angle; azimuth; speed; launch_from; mass; drag_coeff; cross_section } :: rest ->
      let a  = je_expr env angle in
      let az = match azimuth with Some e -> je_expr env e | None -> 0.0 in
      let s  = je_expr env speed in
      let (x, y, z, t) = match launch_from with
        | None               -> (0.0, 0.0, 0.0, 0.0)
        | Some (ex,ey,ez,et) -> (je_expr env ex, je_expr env ey,
                                 je_expr env ez, je_expr env et)
      in
      let pv : projectile_val = {
        angle = a; azimuth = az; speed = s;
        launch_from   = (x, y, z, t);
