...
let rk4_step state dt gravity mass drag_params drag_coeff cross_section =
  ...
  let k1 = { x=deriv.x*.dt; y=deriv.y*.dt; vx=deriv.vx*.dt; vy=deriv.vy*.dt } in
  let s2 = { x=state.x+.k1.x/.2.; y=state.y+.k1.y/.2.;
             vx=state.vx+.k1.vx/.2.; vy=state.vy+.k1.vy/.2. } in
  let d2 = derivatives s2 gravity mass drag_params drag_coeff cross_section in
  let k2 = { x=d2.x*.dt; y=d2.y*.dt; vx=d2.vx*.dt; vy=d2.vy*.dt } in
  let s3 = { x=state.x+.k2.x/.2.; y=state.y+.k2.y/.2.;
             vx=state.vx+.k2.vx/.2.; vy=state.vy+.k2.vy/.2. } in
  let d3 = derivatives s3 gravity mass drag_params drag_coeff cross_section in
  let k3 = { x=d3.x*.dt; y=d3.y*.dt; vx=d3.vx*.dt; vy=d3.vy*.dt } in
  let s4 = { x=state.x+.k3.x; y=state.y+.k3.y;
             vx=state.vx+.k3.vx; vy=state.vy+.k3.vy } in
  let d4 = derivatives s4 gravity mass drag_params drag_coeff cross_section in
  let k4 = { x=d4.x*.dt; y=d4.y*.dt; vx=d4.vx*.dt; vy=d4.vy*.dt } in
...
type state3d = {
  ...
  vx3 : float; vy3 : float; vz3 : float;
}

(* Returns acceleration components for a 3D state.
   Gravity acts only on y (vertical). Drag uses 3D velocity magnitude. *)
let derivatives3d (s : state3d) gravity mass (dp : drag_params) drag_coeff cross_section =
  if dp.enabled then begin
    ...
    if v_mag = 0.0 then
      ...
      let f_mag = 0.5 *. dp.air_density *. v_mag *. v_mag *. drag_coeff *. cross_section in
...
let rk4_step3d (s : state3d) dt gravity mass dp drag_coeff cross_section =
  let drv d = { x3=d.x3*.dt; y3=d.y3*.dt; z3=d.z3*.dt;
                vx3=d.vx3*.dt; vy3=d.vy3*.dt; vz3=d.vz3*.dt } in
  let add_k st k sc = { x3  = st.x3  +. k.x3  *. sc;
                         y3  = st.y3  +. k.y3  *. sc;
                         z3  = st.z3  +. k.z3  *. sc;
                         vx3 = st.vx3 +. k.vx3 *. sc;
                         vy3 = st.vy3 +. k.vy3 *. sc;
                         vz3 = st.vz3 +. k.vz3 *. sc } in
  let d1  = derivatives3d s  gravity mass dp drag_coeff cross_section in
  let k1  = drv d1 in
  let d2  = derivatives3d (add_k s k1 0.5) gravity mass dp drag_coeff cross_section in
  let k2  = drv d2 in
  let d3  = derivatives3d (add_k s k2 0.5) gravity mass dp drag_coeff cross_section in
  let k3  = drv d3 in
  let d4  = derivatives3d (add_k s k3 1.0) gravity mass dp drag_coeff cross_section in
  let k4  = drv d4 in
...
(* ── simulate_trajectory_3d ─────────────────────────────────────────
   ...
   ─────────────────────────────────────────────────────────────────── *)
let simulate_trajectory_3d
    ?(max_time=100.0)
    ?(dt=0.016)
    ?(drag_params=no_drag)
    ~angle ~azimuth ~speed ~x0 ~y0 ~z0 ~gravity
    ~mass ~drag_coeff ~cross_section () =
  ...
  let rec sim t st acc =
    if t > max_time || st.y3 < 0.0 then List.rev acc
      ...
      let next = rk4_step3d st dt gravity mass drag_params drag_coeff cross_section in
...
let simulate_trajectory
    ...
    ~mass ~drag_coeff ~cross_section () =
  let initial_state = { x=x0; y=y0; vx=vx angle speed; vy=vy angle speed } in
...
let max_range_with_drag speed gravity x0 y0 drag_params mass drag_coeff cross_section =
  ...
  for ang = 1 to 89 do
    ...
    if r > !best_range then begin best_range := r; best_angle := angle end
  done;
  ...
  for i = 0 to steps - 1 do
    ...
    if t2 > t1 then begin
      ...
      if area > !best then begin
        best := area; best_x1 := x1; best_x2 := x2; best_h := h
      end
    end
  done;
