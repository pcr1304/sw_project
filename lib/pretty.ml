...
let pp_proj_fields i azimuth launch_from mass drag_coeff cross_section =
  ...
  let lf = match launch_from with
    ...
    | Some (x,y,z,t) ->
        let base = "\n" ^ ind (i+1) ^ "launch_from ("
          ^ print_expr x ^ ", " ^ print_expr y ^ ", " ^ print_expr z in
...
let rec print_sim_stmt i s =
  match s with
  | SProjectile { name; angle; azimuth; speed; launch_from; mass; drag_coeff; cross_section } ->
      let (az, lf, m, d, cs) = pp_proj_fields i azimuth launch_from mass drag_coeff cross_section in
...
let rec print_stmt i s =
  match s with
  | Projectile { name; angle; azimuth; speed; launch_from; mass; drag_coeff; cross_section } ->
      let (az, lf, m, d, cs) = pp_proj_fields i azimuth launch_from mass drag_coeff cross_section in
      ind i ^ "projectile " ^ name ^ " {\n"
      ^ ind (i+1) ^ "angle " ^ print_expr angle ^ "\n"
      ^ ind (i+1) ^ "speed " ^ print_expr speed
